...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getSupportedFormat(gl: GLContext, internalFormat: number, format: number, type: number) {
    ...
    function supportRenderTextureFormat(gl: GLContext, internalFormat: number, format: number, type: number) {
    ...
    function updateColors(dt: number) {
        ...
        if (colorUpdateTimer >= 1) {
            ...
            pointers.forEach(p => {
                p.color = Object.values(generateColor()) as [number, number, number];
            });
        }
    }
    ...
    function step(dt: number) {
        ...
        curlProgram.bind();
        gl.uniform2f(curlProgram.uniforms.texelSize, 1.0 / velocity.read.width, 1.0 / velocity.read.height);
        gl.uniform1i(curlProgram.uniforms.uVelocity, velocity.read.attach(0));
        blit(curl);
    
        vorticityProgram.bind();
        gl.uniform2f(vorticityProgram.uniforms.texelSize, 1.0 / velocity.read.width, 1.0 / velocity.read.height);
        ...
        gl.uniform2f(divergenceProgram.uniforms.texelSize, 1.0 / velocity.read.width, 1.0 / velocity.read.height);
        ...
        gl.uniform2f(pressureProgram.uniforms.texelSize, 1.0 / velocity.read.width, 1.0 / velocity.read.height);
        ...
        gl.uniform2f(gradienSubtractProgram.uniforms.texelSize, 1.0 / velocity.read.width, 1.0 / velocity.read.height);
        ...
        gl.uniform2f(advectionProgram.uniforms.texelSize, 1.0 / velocity.read.width, 1.0 / velocity.read.height);
        if (!ext.supportLinearFiltering)
            gl.uniform2f(advectionProgram.uniforms.dyeTexelSize, 1.0 / velocity.read.width, 1.0 / velocity.read.height);
        ...
        if (!ext.supportLinearFiltering)
            gl.uniform2f(advectionProgram.uniforms.dyeTexelSize, 1.0 / dye.read.width, 1.0 / dye.read.height);
    ...
    function render(target: any) {
        ...
        const L =dye.read.width / dye.read.height;
    ...
    function updatePointerDownData(pointer: Pointer, id: number, posX: number, posY: number) {
        ...
        pointer.deltaX = 0;
        pointer.deltaY = 0;
        pointer.color = Object.values(generateColor()) as [number, number, number];
    }
  ...
  return (
    <div className="fixed top-0 left-0 z-50 pointer-events-none w-screen h-screen">
      <canvas ref={canvasRef} className="w-full h-full block pointer-events-none"></canvas>
    </div>
  );
}
