...
class Title {
  ...
  createMesh() {
    ...
    const textHeight = (this.plane.scale as Vec3).y * 0.15;
    const textWidth = textHeight * aspect;
    this.mesh.scale.set(textWidth, textHeight, 1);
    this.mesh.position.y = -(this.plane.scale as Vec3).y * 0.5 - textHeight * 0.5 - 0.05;
...
class Media {
  ...
  createShader() {
    ...
    img.onload = () => {
      texture.image = img;
      (this.program.uniforms.uImageSizes.value as number[])[0] = img.naturalWidth;
      (this.program.uniforms.uImageSizes.value as number[])[1] = img.naturalHeight;
    };
  }
  createMesh() {
  ...
  update(scroll: { current: number, last: number }, direction: 'left' | 'right') {
    ...
    this.program.uniforms.uTime.value = (this.program.uniforms.uTime.value as number) + 0.04;
    this.program.uniforms.uSpeed.value = this.speed;

    const planeOffset = (this.plane.scale as Vec3).x / 2;
  ...
  onResize({ screen, viewport }: { screen?: { width: number; height: number }, viewport?: { width: number; height: number } } = {}) {
    ...
    (this.plane.scale as Vec3).y = (this.viewport.height * (900 * this.scale)) / this.screen.height;
    (this.plane.scale as Vec3).x = (this.viewport.width * (700 * this.scale)) / this.screen.width;
    (this.plane.program.uniforms.uPlaneSizes.value as number[])[0] = (this.plane.scale as Vec3).x;
    (this.plane.program.uniforms.uPlaneSizes.value as number[])[1] = (this.plane.scale as Vec3).y;
    this.padding = 2;
    this.width = (this.plane.scale as Vec3).x + this.padding;
...
class App {
  ...
  createMedias(items: { image: string; text: string, url?: string }[] | undefined, bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
      ...
      { image: `https://picsum.photos/seed/3/800/600?grayscale`, text: 'Studio' },
      { image: `https://picsum.photos/seed/4/800/600?grayscale`, text: 'After Effects' },
      { image: `https://picsum.photos/seed/5/800/600?grayscale`, text: 'Cinema 4D' },
      { image: `https://picsum.photos/seed/6/800/600?grayscale`, text: 'Figma' },
    ];
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    if (!this.isDragging) {
        ...
        const intersects = this.raycaster.intersectMeshes(meshes);
